...
class AztecView extends Component {
  ...
  _onChange(event) {
    ...
    if (onChange) {
      const caretData = AztecInputState.getCurrentCaretData();
      // console.log('this.aztecViewRef.current', this.aztecViewRef.current);
      // console.log('onChange NATIVE EVENT', {
      //   event: event.nativeEvent,
      //   caretData,
      // });
      // console.log('caretData', caretData);
      // Parse the user input using ExpensiMark
      const expensiMarkHtml = parser.replace(event.nativeEvent.text);
      // console.log('expensiMarkHtml', expensiMarkHtml);
      // Insert raw markdown syntax into the HTML

      // Pass the modified event object with the parsed content
      onChange({
        ...event,
        nativeEvent: {...event.nativeEvent, expensiMarkHtml},
      });
    }
  }

  _onSelectionChange(event) {
    // console.log('onSelectionChange NATIVE EVENT', event.nativeEvent);
